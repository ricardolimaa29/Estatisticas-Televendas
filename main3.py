##################################################################### Sistema criado e Desenvolvido unicamente por 👨‍💻 Ricardo Rodrigues Lima #####################################################################
######################################################### Duvidas entre em contato, 📱 WhatsApp: (14) 99723-7199 #################################################################################################
import flet as ft
import pyodbc
...
def main(pagina:ft.Page):
    ...
    confetti_container = ft.Container(
        content=ft.Image(
            src=".\confete.gif",  # Coloque o caminho do seu GIF aqui
    ...
    def total(progress_bar, percent_label):
        global metas_filtradas, tot
        
        # Processando a meta do vendedor
        codigo_excel = codigo[3:]
        excel_meta_total = pd.read_excel(r'L:\Televendas\Sistema Televendas\Arqui\Metas.xls', sheet_name='Meta')
        excel_meta_total['Vendedor'] = excel_meta_total['Vendedor'].astype(str)
        ...
        try:
            ...
            # Executando a consulta para pegar as vendas
            cursor.execute(f"""SELECT n.nu_ped, SUM(n.vl_prod) 
                                FROM (
                                    SELECT n.nu_ped, n.vl_prod,
                                        ROW_NUMBER() OVER (PARTITION BY n.nu_ped ORDER BY n.nu_ped) AS row_num
                                    FROM nota n
                                    JOIN ped_vda p ON p.nu_ped = n.nu_ped AND p.cd_emp = n.cd_emp
                                    WHERE n.cd_vend_lc = '{codigo}'
                                    AND n.dt_ped BETWEEN '2024-10-01' AND '2024-10-31'
                                    AND n.formpgto_completa IS NOT NULL
                                    AND n.origem_pedido = 'T'
                                ) n
                                WHERE n.row_num = 1
                                GROUP BY n.nu_ped; 
                            """)
            vendas = cursor.fetchall()
            total_vendas_valor = sum(row[1] for row in vendas)  # Somando o valor das vendas
            tot = locale.format_string("%.2f", total_vendas_valor, grouping=True)
            
            # Executando a consulta para pegar o valor dos produtos de devolução
            cursor.execute(f"""SELECT SUM(n.vl_prod) AS VALOR_PRODUTO
                                FROM (
                                    SELECT DISTINCT n.nu_ped, n.vl_prod
                                    FROM nota n
                                    JOIN ped_vda p ON p.nu_ped = n.nu_ped AND p.cd_emp = n.cd_emp
                                    WHERE n.cd_vend_lc = '{codigo}'
                                    AND n.dt_ped BETWEEN '2024-10-01' AND '2024-10-31'
                                    AND n.origem_pedido = 'T'
                                    AND n.nfe_pend_retorno IS NOT NULL
                                    AND n.situacao IN ('DV', 'CA')
                                ) AS n;

								
            """)
            devolucoes = cursor.fetchone()  # Pegando apenas o primeiro resultado de SUM
            tot_diferenca = devolucoes[0] if devolucoes[0] is not None else 0  # Verificando se o resultado não é nulo
            print(tot_diferenca)
            # Calculando o total com devoluções subtraídas
            result = total_vendas_valor - tot_diferenca
            
            
            if total_vendas_ao_todo is not None:
                total_vendas_ao_todo.value = f"💰 Total Faturado esse Mês: R$ {locale.format_string('%.2f', result, grouping=True)} / Meta: R$ {locale.format_string('%.2f', meta_value1, grouping=True)}"
            ...
            print(f"Erro ao processar total de vendas: {e}")

    progress_bar = ft.ProgressBar(value=0, color='Green', width=400, height=15)
    percent_label = ft.Text("0.00%", size=20, font_family="Poppins3")
    ...
    content=ft.Stack(
        [
            ft.Image(
                src="L:\Televendas\Sistema Televendas\Arqui\confete.gif",  # Coloque o caminho do seu GIF aqui
                fit=ft.ImageFit.COVER,  # Ajusta para cobrir toda a área
                width=2000,
                height=1000,
            ),
            ft.Text(
        ...
        alignment=ft.alignment.center,  # Centraliza os elementos dentro do Stack
    ),
    ...
    # sort_table() executa nos Headers o filtro de letras A-Z e numeros 0-1000
    ...
    pagina.add(
        ft.Stack(
            [
                layout,
                confetti_container  # Isso garante que o GIF fique sobre o layout.
            ]
        )
    )
